...
class ASMI(BaseInstrument):
    ...
    def _move_z(self, gantry, x, y, z):
        """Move Z; gantry.move_to already waits for Idle via execute_command."""
    ...
    def indentation(self, gantry) -> dict:
        ...
        print(f"[indent] moving to well_top_z={self._well_top_z:.2f} mm")
        ...
        while True:
            ...
            if current_z <= self._z_target:
                print(f"[indent] reached z_target={self._z_target:.2f} mm")
            ...
            # Use next_z (we just moved there) — skip redundant get_coordinates (~0.3s/step)
            ...
            # Displacement = depth from well surface (positive = indented)
            displacement_mm = self._well_top_z - next_z
            if len(measurements) <= 3 or len(measurements) % 10 == 0:
                print(f"  step {len(measurements)}: disp={displacement_mm:.3f} mm  F={force:.3f} N  dF={corrected:.3f} N")
            if len(measurements) % 10 == 0:
                self.logger.info(
                    "Step #%d: disp=%.3f mm, F=%.3f N, dF=%.3f N",
                    len(measurements), displacement_mm, force, corrected,
                )
        ...
        print(f"[indent] done, returning to safe_z={self._safe_z:.1f} mm")
