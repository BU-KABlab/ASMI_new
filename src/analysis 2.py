...
class IndentationAnalyzer:
    ...
    def adjust_force(self, depths: List[float], forces: List[float], p_ratio: float, approx_height: float) -> List[float]:
        ...
        for i in range(len(depths)): 
            ...
            if 0.1 <= depths[i] <= 1.0:
                ...
                ew_array.append(adjusted_force)
            else:
                # For depths outside analysis range, use original force
                ew_array.append(abs(forces[i]))
        return new_array
    ...
    def analyze_well(self, well: str, poisson_ratio: Optional[float] = None, filename: Optional[str] = None) -> Optional[AnalysisResult]:
        ...
        # Apply retrospective end point filtering if using retrospective method
        if contact_method == "retrospective" and hasattr(self, "_retrospective_end_point"):
            # Truncate data at the detected end point
            end_point_offset = self._retrospective_end_point - first_contact_idx
            if end_point_offset < len(depths):
                depths = depths[:end_point_offset]
                forces = forces[:end_point_offset]
                print(f"🔍 Truncated data to {len(depths)} points due to force drop detection")
...
if __name__ == "__main__":
    ...
    def analyze_well_with_contact_method(self, well: str, contact_method: str = "true_contact", poisson_ratio: Optional[float] = None, filename: Optional[str] = None, contact_force_threshold: float = 2.0, retrospective_threshold: float = 0.05) -> Optional[AnalysisResult]:
        ...
        Args:
            ...
            drop_threshold: Force drop threshold for retrospective method (fraction of max force)            
        Returns:
        ...
        if contact_method == "true_contact":
            ...
            # Use enhanced retrospective contact detection with drop detection
            first_contact_idx, end_point_idx = self.find_retrospective_contact_with_drop_detection(
                corrected_forces, z_positions, retrospective_threshold, drop_threshold
            )
            ...
            print(f"🔍 Data will be truncated at index {end_point_idx} due to force drop detection")
            # Store end point for later data filtering
            self._retrospective_end_point = end_point_idx        
        if first_contact_idx is None:
    ...
    def find_retrospective_contact_with_drop_detection(self, corrected_forces: List[float], z_positions: List[float], threshold: float, drop_threshold: float = 0.5) -> Tuple[int, int]:
        """
        Find contact using retrospective detection and detect sudden force drops.
        
        Args:
            corrected_forces: List of corrected forces
            z_positions: List of Z positions
            threshold: Force threshold for contact detection
            drop_threshold: Force drop threshold (as fraction of max force)
            
        Returns:
            Tuple of (contact_point_index, end_point_index) for analysis
        """
        # First find the contact point using retrospective method
        contact_point = self.find_retrospective_contact_point(corrected_forces, threshold)
        
        # Find the maximum force after contact point
        forces_after_contact = corrected_forces[contact_point:]
        if not forces_after_contact:
            return contact_point, len(corrected_forces) - 1
        
        max_force_idx = contact_point + forces_after_contact.index(max(forces_after_contact, key=abs))
        max_force = abs(corrected_forces[max_force_idx])
        
        print(f"🔍 Retrospective contact at index {contact_point}, max force {max_force:.3f}N at index {max_force_idx}")
        
        # Look for sudden force drops after the maximum force
        end_point = len(corrected_forces) - 1  # Default to end
        
        for i in range(max_force_idx + 1, len(corrected_forces)):
            current_force = abs(corrected_forces[i])
            
            # Check if force drops significantly (more than drop_threshold fraction of max force)
            if current_force < max_force * (1 - drop_threshold):
                # Verify this is a sustained drop (not just noise)
                # Check next few points to confirm the drop
                sustained_drop = True
                check_points = min(3, len(corrected_forces) - i - 1)
                
                for j in range(1, check_points + 1):
                    if i + j < len(corrected_forces):
                        future_force = abs(corrected_forces[i + j])
                        if future_force > max_force * (1 - drop_threshold * 0.5):  # Less strict check
                            sustained_drop = False
                            break
                
                if sustained_drop:
                    end_point = i
                    print(f"🔍 Sudden force drop detected at index {i}: force {current_force:.3f}N (drop of {((max_force - current_force)/max_force*100):.1f}%)")
                    break
        
        print(f"🔍 Using data from contact point {contact_point} to end point {end_point}")
        return contact_point, end_point
    ...
    def calculate_uncertainty(self, covariance, n_points):
        """Calculate uncertainty from covariance matrix"""
        if covariance is None:
            return 0
        return np.sqrt(np.diag(covariance)[0]) if len(np.diag(covariance)) > 0 else 0

    def calculate_r_squared(self, y_actual, y_predicted):
